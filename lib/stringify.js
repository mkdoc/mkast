...
function resolve() {
  ...
  return function(key, val) {
    var invalid = key === '_parent' || key === '_prev' || key === '_lastChild'
      ...
      // NOTE: ignoring `null` which would be desirable
      // NOTE: is not currently possible because it breaks listData.start 
      // NOTE: and listData.delimiter
    ...
    if(!val && val !== undefined || val && typeof(val) !== 'object') {
      return val;
    }
