...
function deserialize(node) {
  ...
  n._lastLinkBlank = node.lastLineBlank;
  n._open = node.open;

  if(node.stringContent) {
 ...
 *
 *  For consistency `_string_content` is converted to `stringContent`.
 *
...
function serialize(node) {
  ...
  // document properties
  if(AstNode.is(node, AstNode.DOCUMENT)) {
    // quick test for any keys, Object.keys() is slow
    for(k in node.refmap) {
      break; 
    }
